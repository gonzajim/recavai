...
Retrieval-augmented generation [Lewis et al., 2020] is the natural architecture for such assistants, and the RecavAI system that motivates this work is one of them. But the reliability of a RAG system is bounded by its knowledge base. If the corpus is segmented so that a fragment starts in Article 8 and ends in Article 9, the citation attached to that fragment is ambiguous; if the segmentation discards the heading hierarchy, the system cannot tell a definition (Article 3) from an obligation (Article 8); if the embedding model was trained on English, Spanish legal vocabulary is represented through a lossy lens. These are not hypothetical failure modes: a first benchmark of the production index retrieved the right document for 80 % of questions but the right document *and* page for only 65 % — the evidence is in the corpus, yet a third of the time the system cannot point to where. An audit of that index (2026-09-23) found two mechanisms behind it: the English embedding model (`all-MiniLM-L6-v2`, 256 word-piece limit) silently discarded 41.2 % of the indexed tokens, because Spanish legal text fragments into roughly twice as many word-pieces as English; and 64 % of the fragments carried a wrong page number.
...
The RecavAI regulatory corpus: 69 documents in Spanish (1,215,181 words, 2,955 pages): the consolidated CSDDD and CSRD (as of 18.03.2026) and the ESRS Delegated Regulation, two analyses of the 2025–2026 amendments and a glossary, 41 GRI standards (universal, topic and sector), 11 OECD due-diligence guidances, a theoretical framework and 9 audit questionnaires. The original production index held 9,176 vectors covering 98.4 % of the corpus text (median 95 words per vector, measured 2026-09-23). Documents are parsed with `pdfplumber` (tables to Markdown) and `python-docx`. The corpus is fixed across all conditions. Licensing of GRI and OECD texts is reviewed before release; where redistribution is not permitted, the released artefact contains offsets and hashes rather than text (Appendix D).
...
| Embedding | `all-MiniLM-L6-v2` (EN, 384d); `paraphrase-multilingual-MiniLM-L12-v2` (ML, 384d); `paraphrase-multilingual-mpnet-base-v2` (ML, 768d) | first = original deployment; multilingual E5/BGE-M3 as an extension. **[Author note, 2026-09-23: `paraphrase-multilingual-mpnet-base-v2` truncates at 128 tokens and `all-MiniLM-L6-v2` at 256, so H1.4 as designed confounds language with context length. The deployed system now uses `intfloat/multilingual-e5-small` (384d, 512 tokens). Consider replacing mpnet with e5-small, or adding a length-matched control — a design change that needs the author's decision.]** |
